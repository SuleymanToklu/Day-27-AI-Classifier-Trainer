# app.py - TAM GÜNCEL VE İYİLEŞTİRİLMİŞ KOD
...
def slugify(text):
    """
    Metni dosya sistemi için güvenli bir formata dönüştürür.
    Örn: "Isparta Gülü" -> "isparta_gulu"
    """
    text = text.lower()
...
def download_images(search_term, folder, max_images=40):
    ...
    print(f"'{search_term}' için resimler indiriliyor...")
    
    downloaded_count = 0
    # DDGS context manager ile kullanılıyor
    with DDGS() as ddgs:
...
def setup_dataset(class1_query, class2_query, num_images_per_class, split_ratio=0.8):
    ...
    # Klasör isimlerini arama sorgularından türetiyoruz
    ...
    def split_files(source_dir, train_dir, val_dir, class_name):
    ...
    split_files(temp_class1_dir, base_dir / 'train' / class1_name, base_dir / 'val' / class1_name, class1_name)
    split_files(temp_class2_dir, base_dir / 'train' / class2_name, base_dir / 'val' / class2_name, class2_name)
    
    shutil.rmtree(base_dir / "temp")
    return f"Veri seti oluşturuldu!\n{msg1}\n{msg2}\nKlasörler: '{class1_name}', '{class2_name}'"
...
def train_model(data_dir, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            print(f'{phase} Kayıp: {epoch_loss:.4f} Başarı: {epoch_acc:.4f}')
...
def start_training_wrapper():
    if not os.path.exists('dataset/train'): return "Hata: Önce veri setini oluşturmalısınız."
    global TRAINED_MODEL, CLASS_NAMES
    TRAINED_MODEL, CLASS_NAMES = train_model('dataset')
    return "Eğitim tamamlandı! 'Tahmin Yap' sekmesine geçebilirsiniz."
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tab("Adım 1: Veri Topla"):
        ...
        data_output = gr.Textbox(label="Veri Toplama Durumu", interactive=False)
    ...
    with gr.Tab("Adım 2: Modeli Eğit"):
        ...
        train_output = gr.Textbox(label="Eğitim Durumu", interactive=False)
...
try:
    if os.path.exists(MODEL_PATH) and os.path.exists(CLASS_NAMES_PATH):
        print("Eğitilmiş model ve sınıf isimleri bulundu, yükleniyor...")
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
